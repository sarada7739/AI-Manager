...
### 6.2a 追加調査（2026-09-04、T-030。事実）

ADR-0009 の承認後、利用者が起動した検証用セッション（本ダッシュボード開発中の Claude Code 2.1.241）に対して確認した。値はいずれも合成のプレースホルダ。

- `sessions/<pid>.<sha256>.key` の中身は **JSON** で、フィールドは `peerToken`（32 桁の 16 進文字列）、`pidDomain`、`procStartFt`。111 バイト、改行なし。
- 受信箱（`\.\pipe\LOCAL\cc-msg-<hash>`）へは **1 行目に認証行**、2 行目以降にメッセージ行を送る。区切りは `
`。認証行のトークンは `.key` の `peerToken`。JSON 全体を送ると認証に失敗して接続が閉じる（「パイプが壊れています」）。
- 認証後、未知の形式の行は **接続を切らずに黙って捨てる**。接続状態では形式の当否が分からない。
- メッセージ行の形式は Claude Code 自身が起動ログに出す投函例と同じ:

```text
{"type":"auth","token":"<peerToken>"}
{"type":"user","message":{"role":"user","content":"本文"}}
```

  任意フィールド: `from`（返信先。`uds:` 前置きの受信箱アドレスでなければ「返信不可の相手」として扱われ、保留 / 配信の受領通知は送られない）、`priority`（`"next"` / `"now"`）、`session_id`（指定すると受信側の自セッション ID と照合され、不一致なら捨てる）、`msg_id`、`file_attachments`。`type` が `control` の行は rename / 受領通知 / idle 通知などの制御用で、ダッシュボードからは送らない。
- 受信側では `origin.kind = "peer"`、`from` 未指定なら `"unknown"` として扱われ、公式ドキュメントどおり「他セッションからのメッセージ」の制約（承認の代行不可、設定変更不可、コマンド不実行）と `crossSessionInbound` の保留 / 配信規則が適用される。
- **端から端まで確認済み**: 上記の 2 行を送ると、受信側（自動モードの対話セッション）に「他の Claude セッションからのメッセージ」として本文が配信された。送信側のスクリプトは `sent: openAfterAuth=True openAfterMsg=True` を返し、応答行は無い（配信 / 保留の受領通知は `from` が `uds:` アドレスのときだけ返る）。
