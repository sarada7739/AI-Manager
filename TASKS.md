...
## 進捗サマリ
- 全 33 件 / 完了 33 件 / 進行中 0 件 / 未着手 0 件
- 現在のタスク: **すべて完了**（第 1 段階 read-only + 実機修正 + 配色 + 第 2 段階 F-7 指示送信。第 2 段階の最終レビューは docs/FINAL_REVIEW.md「第 2 段階レビュー」）
- 最終更新: 2026-09-04T07:00:00+09:00
...
| T-032 | ComposeBox の有効化と送信前確認ダイアログ（クライアント） | T-031 | done | 2 | #37 |
| T-033 | 送信の E2E と README / ARCHITECTURE の更新 | T-032 | done | 2 | #38 |
...
- **受け入れ条件**:
  - [x] `sessions/<pid>.<sha256>.key` の内容が認証行 `{"type":"auth","token":…}` の `token` として通ることを確認する（値はどこにも記録しない）— JSON の `peerToken` で認証が通る
  - [x] 認証行の後に送るメッセージ行の JSON 形式を特定し、`docs/RESEARCH.md` §6 に **形式だけ**（値は合成）を追記する — §6.2a
  - [x] 受信側（自動モード）に配信されることを確認した。保留ダイアログの有無は受信側の設定（`crossSessionInbound`）に依存し、本実装では「配信 / 保留は受信側の設定に従う」と README に明記する
  - [x] `codex queue` の到達性は利用者が Codex 未契約のため検証不能。Codex 宛の送信は第 2 段階の範囲外とする（ADR-0009 追記）
  - [x] 検証に使ったスクリプトは `local-data/`（gitignore 済み）に置き、リポジトリに入れない
...
- **受け入れ条件**:
  - [x] 読み取り専用トグルが ON の間は送信できず、理由を隣に表示する（現状維持）
  - [x] 宛先はアカウント / セッションの選択で決まり、`running` 以外は選べない
  - [x] 確認ダイアログに宛先セッション名 / cwd / 本文の先頭 200 文字を出し、「送る」だけを `primary` にする。`Esc` で閉じる。フォーカスはダイアログに閉じ込める
  - [x] 送信結果（配信 / 保留 / 失敗）をヘッダ帯右端の `LiveStatus` と同じ場所に出す
  - [x] DESIGN.md に無い値を使わない。ダイアログは `--shadow-overlay` を使ってよい（§4.3）
...
- **受け入れ条件**:
  - [x] E2E: トグル OFF → 宛先選択 → 本文入力 → ダイアログ表示 → `Esc` で閉じる（実際の送信はモックの API に対して 1 件）
  - [x] README: 送信の前提（Claude Code 2.1.234 以上、受信側の `crossSessionInbound`）、保留ダイアログの説明、使用量が発生する旨
  - [x] ARCHITECTURE.md §5 / §7 の更新（API と信頼境界）
